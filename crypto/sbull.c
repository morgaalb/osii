...
static char key[33];
static int keylen = 0;
module_param_string(key, key, 33, S_IRUSR | S_IWUSR);
		...
		unsigned long nsect, char *buffer, int write)
{
	...
	if (write) {
		if (keylen) {
			for (i = 0; i < nbytes / ccb; i++) {
				crypto_cipher_encrypt_one(tfm,
						dev->data + offset + i * ccb,
						buffer + i * ccb);
			}
		} else {
...
static int __init sbull_init(void)
{
	...
	if (keylen == 0) {
		printk(KERN_WARNING "Starting sbull without crypto: no key\n");
	} else {
		printk(KERN_WARNING "Starting sbull with key %s\n", key);
		tfm = crypto_alloc_cipher("aes", 0, 0);
		crypto_cipher_setkey(tfm, key, strlen(key));
		ccb = crypto_cipher_blocksize(tfm);
	}
